...
-(void)initView {
    ...
    self.frame = CGRectMake(self.origin.x, self.origin.y, 320, SCREEN_HEIGHT-44-35);
...
-(void)request_type:(NSString *)type bu_brand_id:(NSString *)bu_brand_id tag_id:(NSString *)tag_id key:(NSString *)key
{
    ...
    [[PSNetTrans getInstance] get_Key_Category_Screen_list:self type:type bu_brand_id:bu_brand_id tag_id:tag_id key:key];
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == firstList) {
        ...
        if ([self.categoryData count]>0) {;
            ...
            SearchCategoryEntity *entity = [self.categoryData objectAtIndex:select_row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == firstList) {
        ...
        SearchCategoryEntity *entity = [self.categoryData objectAtIndex:indexPath.row];
...
//        }
        ...
        SearchCategoryEntity *entity1 = [entity.last_category_list objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == firstList) {
        ...
        if ([entity.category_name isEqualToString:@"全部商品"]) {//全部商品
            ...
            if (self.block) {
                self.block(nil,nil,entity.category_name,0);
                return;
            }
        }
        ...
        if ([entity.last_category_list count]>0) {
            ...
            if (self.block) {
                self.block(@"1",entity.category_code,entity.category_name ,0);
            }
        }
        ...
        if (self.block) {
            if (indexPath.row == 0) {
                ...
                if ([entity.last_category_list count]>1) {//全部什么什么
                    self.block(@"1",entity.category_code,entity.category_name,1);
                } else {
                    self.block(@"0",entity1.category_code,entity1.category_name,0);
                }
...
{
    if(t_API_KEY_CATEGORY_SCREEN_LIST == nTag)
    {
        [self.categoryData removeAllObjects];
        ...
        firstEntity = [[SearchCategoryEntity alloc] init];
        firstEntity = (SearchCategoryEntity *)[self.categoryData objectAtIndex:0];
...
#pragma mark ----------------
-(void)addFirstEntity {
    SearchCategoryEntity *entity = [self.categoryData objectAtIndex:0];
