...
@implementation CategoryGoodsEntity
@synthesize bu_category_id = _bu_category_id,category_name = _category_name,icon = _icon,has_son = _has_son ,next_cate = _next_cate;
@end


@implementation CategoryNetTransObj
/*
...
-(BOOL) responseSuccess:(ASIHTTPRequest *)request{
    ...
    if([status isEqualToString:WEB_STATUS_1] ){
        for (NSDictionary *dic in datas) {
            CategoryGoodsEntity *goodsList = [[CategoryGoodsEntity alloc] init];
            goodsList.bu_category_id = [dic objectForKey:@"bu_category_id"];
            goodsList.category_name = [dic objectForKey:@"category_name"];
            goodsList.icon = [dic objectForKey:@"icon"];
            goodsList.has_son = [dic objectForKey:@"has_son"];
            NSMutableArray * next_cate = [[NSMutableArray alloc]init ];
            if ([goodsList.has_son isEqualToString:@"1"]) {
                if ([dic objectForKey:@"next_cate"]) {
                    NSMutableArray * childArray = [dic objectForKey:@"next_cate"];
                    for (NSDictionary * d in childArray) {
                        CategoryGoodsEntity *childList = [[CategoryGoodsEntity alloc] init];
                        childList.bu_category_id = [d objectForKey:@"bu_category_id"];
                        childList.category_name = [d objectForKey:@"category_name"];
                        childList.icon = [d objectForKey:@"icon"];
                        childList.has_son = [d objectForKey:@"has_son"];
                        [next_cate addObject:childList];
                    }
                }
                
            }
            goodsList.next_cate = [NSMutableArray arrayWithArray: next_cate];
            [array addObject:goodsList];
        }
