...
@implementation BuListEntity
@synthesize bu_id,bu_name,bu_code,address;
@end


@implementation BuListNetTransObj
/*
...
-(BOOL) responseSuccess:(ASIHTTPRequest *)request{
    ...
    if([status isEqualToString:WEB_STATUS_1] ){
        for (NSDictionary *dic in datas) {
            BuListEntity* collectEntity = [[BuListEntity alloc] init];
            collectEntity.bu_name = [dic objectForKey:@"bu_name"];
            collectEntity.bu_id = [dic objectForKey:@"bu_id"];
            collectEntity.address = [dic objectForKey:@"address"];
            [array addObject:collectEntity];
        }
    }
    else{
                     ...
                     else
                     {
                         if (datas.count > 0)
                         {
                             ...
                             for (NSDictionary *dic in datas)
                             {
                                 ...
                                 collectEntity.bu_id = [dic objectForKey:@"bu_id"];
                                 collectEntity.address = [dic objectForKey:@"address"];
                                 collectEntity.bu_code = [dic objectForKey:@"bu_code"];
                                 [array addObject:collectEntity];
                             }
                             [_uinet responseSuccessObj:array nTag:self._nApiTag];
                             return;
