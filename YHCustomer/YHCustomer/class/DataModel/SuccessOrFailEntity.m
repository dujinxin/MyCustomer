...
@implementation SuccessOrFailEntity
@synthesize _message;
@synthesize _status;
@synthesize _message_count;
@synthesize _data;

+(id)entitySuccessOrFailEntity:(NSDictionary *)dic
{
    SuccessOrFailEntity* entity = [[SuccessOrFailEntity alloc] init];
    entity._message_count = [NSString stringWithFormat:@"%@",[dic objectForKey:@"apply_total"]];
    if ([[dic allValues] count] > 0) {
        entity._data = [NSString stringWithFormat:@"%@",[[dic allValues] objectAtIndex:0]];
    };
    return entity;
}
...
@implementation SuccessOrFailTrans
...
//             [_uinet requestFailed:_nApiTag withStatus:@"0" withMessage:error.domain];
