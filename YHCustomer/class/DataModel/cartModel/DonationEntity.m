...
@implementation DonationEntity

@end


@implementation DonationTrans
/*
...
-(BOOL) responseSuccess:(ASIHTTPRequest *)request{
    ...
    if([status isEqualToString:WEB_STATUS_1] ){
        for (NSDictionary *dic in datas) {
            DonationEntity *entity = [[DonationEntity alloc] init];
            entity.info = [dic objectForKey:@"info"];
            entity.order_list_id = [dic objectForKey:@"order_list_id"];
